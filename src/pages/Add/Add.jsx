...
function Add() {
  ...
  async function submitHandler(e) {
    ...
    e.preventDefault();
    if(skillsTitle.trim() === '') {
      seterrorObj(prevState => ({...prevState, skillsTitle: "Title can't be blank"}))
    }
    if(skillsDescription.trim() === '') {
      seterrorObj(prevState => ({...prevState, skillsDescription: "Description can't be blank"}))
    }
    const newSkills ={
      title: skillsTitle,
  ...
  return (
    ...
    <form onSubmit={submitHandler} className={css.form}>
        ...
        <input onChange={(e) => setSkillsTitle(e.target.value)} value={skillsTitle} className={`${css.input} ${errorObj.skillsTitle ? css.errBg: ''}`} type="text" placeholder='Title' />
